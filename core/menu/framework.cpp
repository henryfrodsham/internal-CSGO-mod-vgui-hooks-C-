...
void menu_framework::tab(std::int32_t x, std::int32_t y, unsigned long font, const std::string string, std::int32_t& tab, std::int32_t count) {
	GetCursorPos(&cursor); ScreenToClient(GetForegroundWindow(), &cursor);

	if ((cursor.x > x) && (cursor.x < x + 100) && (cursor.y > y) && (cursor.y < y + 10) && (GetAsyncKeyState(VK_LBUTTON) & 1))
		tab = count;
	if (tab == count) {
		render::draw_filled_rect(x, y, 100, 10, color::gui_hint_1());
	}
	else {
		render::draw_filled_rect(x, y, 100, 10, color::gui_hint_4());
	}
	render::text(x + 50, y - 2, font, string, true, color::gui_text_2());
}
...
void menu_framework::check_box(std::int32_t x, std::int32_t y, unsigned long font, const std::string string, bool& value) {
	...
	render::text(x + 9, y - 3, font, string, false, color::gui_text_1());
}
...
void menu_framework::selection_interface(std::int32_t x, std::int32_t y, selections selections, std::int32_t& current, bool& opened) {
	...
	if (opened) {
		...
		for (short int pointer = 0; pointer < 6; pointer++) {
			...
			if ((cursor.x > x) && (cursor.x < x + 50) && (cursor.y > y + 10 + pointer * 10) && (cursor.y < y + 20 + pointer * 10) && (GetAsyncKeyState(VK_LBUTTON) & 1)) {
				current = pointer;
				opened = !opened;
			}
			render::draw_filled_rect(x, y + 10 + pointer * 10, 50, 10, color::gui_hint_4());
			render::text(x + 25, (y + 10 + pointer * 10) - 2, render::fonts::watermark_font, selections.deref(pointer), true, color::gui_text_2());
			render::draw_line(x, y + 10 + pointer * 10, x + 49, y + 10 + pointer * 10, color::gui_hint_2());
			render::draw_line(x, y + 20 + pointer * 10, x + 49, y + 20 + pointer * 10, color::gui_hint_2());
		}
	}
	else {
		render::draw_filled_rect(x, y, 50, 10, color::gui_hint_4());
	...
	if (opened) {
		...
		for (short int pointer = 0; pointer < 6; pointer++) {
			...
			if (current.deref(pointer)) {
				render::draw_filled_rect(x, y + 10 + pointer * 10, 50, 10, color::gui_hint_3());
			}
			else {
				render::draw_filled_rect(x, y + 10 + pointer * 10, 50, 10, color::gui_hint_4());
			}
