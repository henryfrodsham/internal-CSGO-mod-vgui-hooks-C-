...
void aimbot::master::aim_at_target(c_usercmd* cmd) {
	...
	vec3_t local_origin = (csgo::local_player->origin() + csgo::local_player->view_offset()); vec3_t dif = AimAt - local_origin;
	...
	vec3_t view_punch = csgo::local_player->punch_angle();
...
player_t* aimbot::sub::get_best_target() {
	...
	for (short int entId = 0; entId < 64; entId++) {
		...
		if (variables::aimbot::fovcheck && entity_distance > variables::aimbot::fovcircle && variables::aim_priority == 1) {
			continue;
		}
...
/*
bool aimbot::sub::aiming_at(player_t* entity) {
	trace_t tr;ray_t ray;trace_filter filter;filter.skip = csgo::local_player;
	auto start = csgo::local_player->get_eye_pos();
	auto end = entity->
	ray.initialize(start,);
	interfaces::trace_ray->clip_ray_to_entity(ray, MASK_SHOT, csgo::local_player, &tr);
}
*/
