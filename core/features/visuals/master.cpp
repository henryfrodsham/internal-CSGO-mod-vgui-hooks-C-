...
vec2_t visuals::master::ViewAnglesToScreen(vec2_t angle) {
	vec3_t view_angles; interfaces::engine->get_view_angles(view_angles); vec2_t angles = { view_angles.x,view_angles.y };
	int x, y;; interfaces::engine->get_screen_size(x,y);
	vec2_t difference = angles - angle;
	return { 0,0 };
}
void visuals::modules::ShowTargetInfo() {
	...
	vec3_t velocity = variables::aimbot_target->velocity(); int vel;
	vel = abs(velocity.x) + abs(velocity.y);
